...
int main(){
    ...
    vector<double> C_1(meshNum) ;
    vector<double> C_2(meshNum) ;

    for (int ii = 0; ii < meshNum; ii++)
    {
        if (ii!=0&&ii!=meshNum-1)
        {
            C_1[ii] = (2*D[ii]*D[ii+1])/(d_x*D[ii+1]+d_x*D[ii]) ;
            C_2[ii] = (2*D[ii]*D[ii-1])/(d_x*D[ii-1]+d_x*D[ii]) ;
        }else{
            C_1[ii] = 0. ;
            C_2[ii] = 0. ;
        }
    ...
    for (int ii = 0; ii < repeat_max; ii++)
    {
        ...
        for (int jj = 0; jj < meshNum; jj++)
        {
            if (jj == 0)
            {
                ...
                temp = (C_1[jj]*phi_1[jj+1]+C_2[jj]*phi_1[jj-1]+S[jj]*d_x)/(C_1[jj]+C_2[jj]+sigma_a[jj]*d_x) ;
            }
